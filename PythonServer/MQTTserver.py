...
"""
import json
mqtt_topic_cmds = "MM/metofficeAscot/MMQTRelay"
...
client = mqtt.Client(client_id="MQTT02")
...
while True:
    ...
    now = dt.now()
    t = now.strftime("%Y.%m.%d.%H.%M.%S.%w")
    timestamp = '{"cmd":"set","var":"localtime","val":"'+t+'.-1.-1"}'
    ...
    client.publish(mqtt_topic_cmds, payload=timestamp, qos=1, retain=False)
